...
const Checkout = () => {
  ...
  const handleAccountNext = (e: React.FormEvent) => {
    ...
    // Free plans skip billing â€” go to summary
    if (plan.isFree) {
      setActiveStep(1);
      return;
    }
                          ...
                          return (
                            ...
                            >
                              ...
                              <Typography variant="caption" sx={{ display: "block", color: isActive ? "grey.400" : "grey.500", fontSize: "0.65rem", lineHeight: 1.2 }}>
                                {key === "starter" ? "Custom" : `${p.price}${p.period}`}
                              </Typography>
                            </Box>
                          );
                        })}
                    ...
                    <Box>
                      ...
                      <Box sx={{ mb: 2, p: 2, bgcolor: "grey.50", borderRadius: 2, border: "1px solid", borderColor: "divider" }}>
                        <Stack spacing={0.75}>
                          {[
                            ...
                            ...(!plan.isFree ? [
                              { label: "Card", value: `â€¢â€¢â€¢â€¢ ${billingInfo.cardNumber.replace(/\s/g, "").slice(-4) || "â€¢â€¢â€¢â€¢"}` },
                              { label: "Expiry", value: billingInfo.expiry || "â€”" },
                            ] : []),
                          ].map((item) => (
                            <Box key={item.label} sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                              <Typography variant="caption" sx={{ color: "text.secondary" }}>{item.label}</Typography>
                              <Typography variant="caption" sx={{ fontWeight: 600, color: "text.primary" }}>{item.value || "â€”"}</Typography>
                            </Box>
                          ))}
                        </Stack>
                      </Box>
                      ...
                      <Box sx={{ mb: 2, p: 2, bgcolor: "#0891B20D", borderRadius: 2, border: "1px solid", borderColor: "#0891B233" }}>
                        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 0.75 }}>
                          ...
                          <Typography variant="caption" sx={{ fontWeight: 600, color: "text.primary" }}>{plan.name} â€” {plan.isFree ? "14-day trial" : "Monthly"}</Typography>
                        </Box>
